...
const Navbar: React.FC = () => { const [open, setOpen] = useState(false); const go = (id: string) => { setOpen(false); scrollToSection(id); }; return <header className="wrap site-nav">
    ...
    <a className="nav-cta" href="#contact" onClick={(e) => { e.preventDefault(); scrollToSection("contact"); }}>Let&apos;s talk ↗</a>
    <button className="nav-toggle" aria-label="Open navigation" aria-expanded={open} onClick={() => setOpen(!open)}>☰</button>
