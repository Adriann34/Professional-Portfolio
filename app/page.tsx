...
const experience = [
  ...
  },
  {
    ...
    company: "Freelance",
    date: "2025 — 2026",
    description: "Built landing pages and custom web systems for clients, turning business needs into responsive interfaces, streamlined workflows, and polished digital experiences.",
  },
  {
    role: "Full Stack Web Developer",
...
export default function Home() {
  ...
  return <>
    ...
    <main id="main">
      ...
      <section id="about" className="about shell section-space"><div className="section-label reveal"><span className="section-pill">A little about me</span></div><div className="section-heading reveal"><h2>Engineering the details,<br /><span>Seeing the bigger picture.</span></h2></div><div className="about-grid reveal"><Slideshow /><aside className="about-aside"><div><span className="meta-label muted">THE APPROACH</span><h3>Curiosity.<br />Craft.<br />Ownership.</h3></div><p>I think through the problem first, then build polished interfaces that are clear and easy to use.</p><a className="text-link" href="https://renew-marketplace.vercel.app/" target="_blank" rel="noreferrer">Meet my latest project <ArrowUpRight size={17} /></a></aside></div></section>
      <section className="capabilities shell section-space" aria-labelledby="services-title"><div className="section-label reveal"><span className="section-pill">What I offer</span></div><div className="capability-heading section-heading reveal"><h2 id="services-title">Full stack, end to end.</h2></div><div className="capability-grid">{capabilities.map(({ name, text, tag, image }, index) => <article className={`capability reveal ${index === 2 ? "dark-card" : ""}`} key={name}><div className="capability-content"><div className="capability-copy"><span className="capability-tag">{tag}</span><h3>{name}</h3><p>{text}</p></div></div><div className="capability-visual"><img src={image} alt="" loading="lazy" /></div></article>)}</div></section>
      ...
      <section id="work" className="work shell section-space"><div className="section-label reveal"><span className="section-pill">Portfolio</span></div><div className="section-heading reveal"><h2>A few things I’ve built.</h2></div><div className="project-gallery reveal">{projects.map((project) => <ProjectCard project={project} key={project.title} />)}</div></section>
    </main>
